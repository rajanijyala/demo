...
const VerifyRegisterOtp = () => {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      const result = await verifyRegisterOtp(values);
    ...
    <main className="mx-auto flex min-h-[75vh] max-w-4xl items-center justify-center px-6 py-12 lg:px-8">
      <motion.section initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="w-full rounded-3xl border border-slate-800 bg-slate-900/80 p-8 shadow-2xl shadow-slate-950/30 lg:p-10">
        <div className="mb-6 text-left">
          <p className="text-sm uppercase tracking-[0.3em] text-sky-200">Verify email</p>
          <h1 className="mt-3 text-3xl font-semibold text-white">Enter your registration OTP</h1>
          <p className="mt-2 text-slate-300">Use the 6-digit code sent to your email.</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 text-left">
          ...
          <Button type="submit" className="w-full" disabled={isSubmitting}>{isSubmitting ? 'Verifying...' : 'Verify and continue'}</Button>
          <p className="text-sm text-slate-400">Entered the wrong email? <Link to="/register" className="text-sky-300">Register again</Link></p>
        </form>
      </motion.section>
    </main>
  );
