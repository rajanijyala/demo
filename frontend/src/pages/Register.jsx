...
const Register = () => {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      navigate('/verify-register-otp', { state: { email: values.email, devOtp: result.devOtp } });
    } catch (err) {
    ...
    <main className="mx-auto flex min-h-[75vh] max-w-5xl items-center justify-center px-6 py-12 lg:px-8">
      <motion.section initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="grid w-full gap-8 rounded-3xl border border-slate-800 bg-slate-900/80 p-8 shadow-2xl shadow-slate-950/30 lg:grid-cols-[0.9fr_1.1fr] lg:p-10">
        <div className="space-y-4 text-left">
          ...
          <h1 className="text-3xl font-semibold text-white">Verify your email before entering the dashboard</h1>
          <p className="text-slate-300">Create your account and receive a 6-digit OTP by email.</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 rounded-3xl border border-slate-800 bg-slate-950/80 p-6 text-left">
          ...
          <Input label="Confirm password" type="password" placeholder="********" {...register('confirmPassword', { required: 'Confirm your password', validate: (value, formValues) => value === formValues.password || 'Passwords do not match' })} error={errors.confirmPassword?.message} />
          <Button type="submit" className="w-full" disabled={isSubmitting}>{isSubmitting ? 'Sending OTP...' : 'Create account'}</Button>
